...
def sgd_unweighted(*args):
    return _s_gd2.sgd_unweighted(*args)
sgd_unweighted = _s_gd2.sgd_unweighted
...
def layout_scipy_unweighted(S, t_max=15, mu_min=.1):
    """takes a scipy sparse matrix S with n vertices, using unit path lengths,
    ...
    # perform SGD
    sgd_unweighted(X, I, J, t_max, mu_min)
    return X



def layout_scipy_weighted(S, t_max=15, mu_min=.1):
    ...
    d = shortest_path(S, directed=False, unweighted=True)
    d = squareform(d)
    w = 1/d**2
